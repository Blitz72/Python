...
def set_power(group, values):
    ...
    for value in values:
        print(va.speak(f'turn <emphasis level="{emphasis_level}"> {value} </emphasis> the {group}, please'))
        sleep(15)
...
def set_brightness(group, values):
    for value in values:
        if value == 'dim' or value == 'brighten':
            ...
            launch_word = 'set'
            emphasis_level = 'none'
        print(va.speak(f'<emphasis level="{emphasis_level}"> {launch_word} </emphasis> the {group} {added_str}, please'))
        sleep(15)
